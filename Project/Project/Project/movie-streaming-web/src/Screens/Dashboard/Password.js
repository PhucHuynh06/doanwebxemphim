...
import { changePassworAction } from '../../Redux/Actions/userActions';
...
//on submit
const onSubmit = (data) =>{
  dispatch(changePassworAction(data));
};
...
}, [ isSuccess, isError, reset, dispatch, message]);
  ...
  return (
    <SideBar>
      <form onSubmit={
        ...
        <div className='w-full'>
            <Input
              label="Xác nhân lại mật khẩu mới"
              placeholder="**************"
              type="password"
              bg={true}
        ...
        <div className="flex justify-end items-center my-4">
          <button
           ...
           className="bg-main font-medium transitions hover:bg-subMain border border-subMain text-white py-3 px-6 rounded w-full sm:w-auto">
            {
              isLoading ? "Changing..." : "Thay đổi mật khẩu"
            }
          </button>
        </div>
      </form>
