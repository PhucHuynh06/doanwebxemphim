    ...
    >
      {movies?.slice(0, 6).map((movie, index) => (
        <SwiperSlide key={index} className='relative rounded overflow-hidden'>
          <img
            src={movie?.image ? movie.image : 'imgaes/user.png'}
            alt={movie?.name}
    ...
    <div className='relative w-full'>
      {isLoading ? (
        ...
        <div className='mt-6 flex justify-center items-center'>
          <div className={sameClass}>
            <div className='w-24 h-24 p-5 mb-4 rounded-full bg-dry text-subMain text-4xl flex justify-center items-center'>
              <Ri24HoursLine />
            </div>
            <p className='text-border text-sm text-center'>
              Hiện Tại Danh Sách Phim Đang Trống
            </p>
          </div>
        </div>
      )}
